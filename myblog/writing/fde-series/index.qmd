...
![](fde_main.jpg){fig-alt="An engineer arriving through a doorway into a busy workplace, with a clock above the entrance."}
...
Most of the examples here come from AI and machine-learning engagements, because that is where I have done this work. The delivery model is broader, but the claims that follow are grounded in that experience.

I've written before, in [Hypothesis-Driven AI Delivery](../hypothesis-driven-ai-delivery/), about how I frame this type of work before it starts: getting clear on the
...
**Should the work be done this way at all?** Forward deployed engineering has preconditions, and where they are absent, the critics who label it expensive consulting are right. Naming those conditions lets us decide when this model fits and when something else would serve better — and what, if it does fit, the customer actually gets.
...
The sections are numbered, but they don't queue up neatly. The handover question has to be answered before the building starts, and the prioritisation question gets asked every week. What they share is the constraint above, and each section is a position I've arrived at by getting it wrong at least once.
...
![](fde1.jpg){fig-alt="A figure at a crossroads of several lit paths, consulting a checklist before taking the one marked out ahead."}
...
The distinction can have a commercial shape, but the contract form does not define it. Time-and-materials work can have variable scope too. The difference is what governs change. In scoped implementation, the agreed deliverable is the primary focus, and departures from it are changes to manage. Here, the outcome is the primary focus: capacity and duration are fixed, scope is expected to move, and the joint team reprioritises one shared backlog as evidence emerges. The engagement ends when the capacity is spent or the term expires, whichever comes first — not when a predetermined list is finished.

The quickest test of whether a requirement is genuinely descriptive is to **ask what would be different in the business at the end, and how we would know.** If the answer is a list of things to build, or a platform to prove, the requirement is prescriptive whatever language surrounds it — the deliverable is the requirement, and the outcome focus is just how it was sold.
...
What we commit is capacity and duration. What we steer by is the outcome. Those are not the same thing, and customers hear the second one louder; especially when FDE is positioned as outcome-led. If you tell a customer that the engagement will be judged on whether a business metric moves, it is reasonable for them to hear a commitment that the metric *will* move. What's on offer is a fixed team for a fixed period, pointed at that outcome. Most of the challenges I've seen relating to setting expectations start from this misalignment between perceived commitments and actual commitments.

I can completely understand this from the customer's perspective. If you are paying for an expensive team to come in and help you deliver an outcome, it takes a lot of trust to proceed if all that team commits to is an honest account at the end of how far it got and what it would take to finish. However, the outcome is lagging: it often lands after we've gone and depends on more than us, which is why it can't be the commitment.

The distinction I now make is between measures that tell you the outcome landed and measures that drive it. Inventory down, churn down, a forecasting capability the customer runs themselves — those are lagging, and they go in as shared direction. What the team is held to are the leading measures: things the joint team produces and controls inside the window, chosen as the best available drivers of the lagging ones. The lagging outcome's job is to keep the leading measures honest, not to become a penalty on a team for results it doesn't control.

On a demand-forecasting engagement, the leading measures might be: a baseline for the current process, measured by week four; an evaluation gate the customer's own experts have calibrated, and that can stop or approve a release; a model in production by week twelve, serving every SKU in the pilot stores; three of the customer's engineers shipping changes through that gate without us; monitoring and a runbook live. Each of those should have a reason it brings inventory down. If you can't give one, you're measuring activity, or you've missed a driver.

The targets have to be realistic. We're timeboxed, and we should only put a number down if we're confident it's reachable inside the window. We won't always hit them. Missing one narrowly is an easy conversation. Landing only a third of the list isn't, and that's ours to answer for. Naming them is how we put skin in the game, especially with customers who haven't worked with us before. They can be contractual — milestones, acceptance, payment — and, like everything else, they're revisited as the engagement teaches us more.

Most of that list is capability rather than software. A baseline the customer owns, a gate their experts calibrated, a runbook they operate, three engineers shipping without us — those are things the organisation can do at the end that it couldn't at the start. That's what co-engineering is designed to buy beyond the software itself. Taken together, the leading measures make concrete both what is delivered and what is transferred; the outcome is what the customer goes on to do with them once we've gone. A customer who only wanted the model has cheaper ways to get one.
...
So the fix isn't a clever form of words. It's to keep saying it: the outcome is what we're aiming at and what we'll measure against, the leading measures are what we're delivering and what we'll be judged on, and the capacity and the window are what bound both. Most of the trouble I've seen comes from saying it once, at the start, and treating that as done.

### Somebody has to own the outcome

The question to ask is who owns the outcome once we've gone. The answer has to be a name, not a plan to find one: a person inside the customer's organisation who is accountable for the outcome and involved while the work is happening. Not just a sponsor who receives updates — someone whose result this is, and who will act on what the engagement establishes. That role cannot be delegated outward: a vendor cannot own your outcome, and a product certainly can't.

This is a different question from who maintains and evolves the system afterwards, which may have several legitimate answers — their own team, a partner, or something they buy — each implying a different way of structuring the engagement. The owner, though, is the part that has to be true before anything starts, regardless of how the contract describes the work. If there is nobody accountable on their side, the engagement will produce something impressive that decays quietly. I've done that. It's a worse outcome than not starting, and whatever our titles said, we were contractors with a better title.
...
Building everything from scratch, you can get something simple into production inside a short window. The problems forward deployed engineering gets pointed at aren't simple. Getting one of those there with the gate, the monitoring, the runbook and the engineers shipping through it — the parts that survive you leaving — is the bespoke work. Do it from scratch and you are also building the platform it runs on, none of which is specific to the problem. A small team can't afford both inside the window. What makes the bespoke layer affordable is everything underneath it that you didn't have to build: managed model platforms, data platforms, tooling, hosting, orchestration. That substrate is the reason a small team can do this at all.
...
Every viable version of this model I've seen has that substrate. You either build it — an enormous, sustained investment that almost nobody can now justify per customer — or you stand on someone else's. What changed over the last few years is that standing on someone else's became genuinely viable. The model — the piece that used to be the bulk of the bespoke work — became something you call rather than build, and the platforms around it became things a customer already has or can switch on, priced by use rather than by a build. I think that's a large part of why this model has spread as fast as it has.
...
This is the most interesting work I've done. You end up deep inside somebody else's domain — how a manufacturer decides a part is defective, how a stylist thinks about which items are complementary, what a support engineer actually knows that has never been written down anywhere. You have to understand it well enough to collaborate with people who do it for a living. That's a real privilege, and it's the part I'd miss most.
...
*You have a fixed window of time inside someone else's organisation. Everything else follows from that.*

![](fde2.jpg){fig-alt="Two engineers working side by side on a factory floor beneath a large countdown clock."}

The clock creates a strong instinct: start building immediately. Time is short, the sponsor is watching, and code — now cheap to generate — is the easiest output to mistake for progress. I've followed that instinct and watched other people follow it, and I now believe it is usually wrong. Inside a fixed window, the highest-leverage early work is often understanding how the customer's work is done today, and instrumenting what the existing process fails to record, rather than rushing to build the proposed solution.
...
My standard is blunt: if it can't stop a release, it's a dashboard, not a gate. It is also one of the leading measures I'd put a target on at the start. Whether that gate exists and can stop a release is ours to deliver inside the window, whatever happens to the outcome afterwards.
...
That is the job. Not the title but the constraint: a fixed window, someone else's organisation, and a system that has to outlive the team that built it.

All of that is about the system: what to capture, what to measure it against, and how to let autonomy
follow the evidence. That is deliberately half the story. The other half is the organisation it happens inside — its people, its politics, its own view of why you are there — and a window that will not stretch to everything on the list.

## 03 · The Joint Call

*How two teams with one deadline decide what to build together — and what happens to everything else*

![](fde3.jpg){fig-alt="A joint team gathered around a shared board of backlog columns, one person moving an item."}
...
During the work, the most valuable customer contribution is often deep knowledge of the business and domain: how the process really operates, what has already been attempted, where decisions become blocked, which constraints are fixed, and who can move the ones that are not. Customer engineers are valuable when they bring that knowledge or will eventually own the system — and where they will, working alongside them is how the capability transfers, which is why "their engineers shipping without us" belongs on the list of leading measures. They are not, though, the only evidence of commitment.
...
The steering committee is where that sponsorship becomes active. It shouldn't be a status report, but a working session: what the team is doing now, what is blocked, what's next, and where the sponsor needs to change the order or break a tie. It is also where the leading measures agreed at the start are reviewed against their targets — the honest account of distance travelled, given while there is still time to change course rather than at the end.
...
The uncertain question was whether the system could support multiple journeys. If it couldn't, there was little value in productionising the first one. That was the leading measure. The outcome the sponsor cared about depended on it, and it was the one thing we could still settle inside the window. The customer team already knew how to navigate their own governance, even if it was slow, and it was something that team had done many times.
...
Some work simply fell away. We had wanted to be there when the first real traffic arrived, so we could refine monitoring and observability against live data. We never got that chance. It was a straightforward cost of a window that wasn't big enough.
...
![](fde4.jpg){fig-alt="A departing engineer handing a glowing, intricate object into another person's open hands."}
...
That isn't a playbook you can apply mechanically, and I'd be wary of anyone selling it as one. It's a set of decisions that keeps coming round, and a set of mistakes I've made often enough to recognise on the way in.
...
The critics say this is consulting with a better title. Sometimes it is. What separates it isn't the outcome — nobody can commit to a result that lands after they've left and depends on people they don't manage, and pretending otherwise is how the expectation problems start. What separates it is that we can name, at the start, the things inside the window that drive that result, put targets on them, and be judged against them. A baseline measured. A gate that can stop a release. Their engineers shipping changes without us. Those are ours to deliver, and if we don't, no title protects us. They are also what the organisation is left holding when we go — the reason to do it with us rather than have it done for them.

That is also what trust is, in this work. Not a feeling somebody extends to you, but a judgement they reach from evidence accumulated together over months — the leading measures met or missed, in the open. It decides whether the thing you built gets used, and it is the part no amount of technical progress has made any easier.
